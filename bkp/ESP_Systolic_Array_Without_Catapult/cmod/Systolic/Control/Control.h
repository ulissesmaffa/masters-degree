...
SC_MODULE(Control) {
 public:
  ...
  // 3--0x30: weight_read_base
  // 4--0x40: input_read_base
  // 5--0x50: input_write_base
  // 6--0x60: flip memory buffer (no IRQ, write only)
  // 7--0x70: interrupt (read only)
  ...
  void ControlRun() {
    ...
    while(1) {
      ...
      if (rv_in.PopNB(rv_in_reg)) {
        // use bit [6-4] for 32-bit = 4-byte word size
        index = nvhls::get_slc<3>(rv_in_reg.addr, 4);
      ...
      // map interrupt to 0x70
      if (is_read && (index==7))
        rv_out_reg.data = interrupt.read(); // read interrupt
