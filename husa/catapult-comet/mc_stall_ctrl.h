...
SC_MODULE(mc_stall_ctrl2) {
  ...
  enum States { RESET, INIT, WAIT, SEEK, STALL2, STALL3, DONE };
  States                    cstate;
      ...
      if (clk.read() == SC_LOGIC_1) {
        switch(cstate) {
          ...
          case INIT: { 
            ...
            holdcnt = hold;
            if (enable_stalls.read() == sc_dt::Log_1) cstate = WAIT; 
            break; 
          }
          case WAIT: {
            if (--intervalcnt == 0) cstate = SEEK; // wait atleast 'interval' clock cycles before seeking a stall pin
            break; 
          }
          case SEEK: {
          ...
          case STALL2: {
            if (--holdcnt == 0) cstate = STALL3; // hold stal for 'hold' clock cycles
            break;
          }
          case STALL3: {
            std::ostringstream msg;
